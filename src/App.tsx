...
const App = () => {
  ...
  const updateInboxFilter = (filter: 'strict' | 'flexible') => {
    setState(prev => ({ ...prev, inboxFilter: filter }));
  };

  const playNotificationSound = useCallback(() => {
    // Custom Sound
    if (state.notificationSound === 'custom' && state.customSoundData) {
      try {
    ...
    // Generated sounds using Web Audio API
    try {
      ...
      if (state.notificationSound === 'retro') {
        playTone(523.25, 'square', now, 0.1); // C5
        playTone(659.25, 'square', now + 0.1, 0.1); // E5
        playTone(783.99, 'square', now + 0.2, 0.2); // G5
      } else if (state.notificationSound === 'soft') {
        playTone(440, 'sine', now, 0.2);
        playTone(554.37, 'sine', now + 0.2, 0.4);
      } else {
        // Default "Glass" sound simulation
        playTone(880, 'sine', now, 0.1);
        playTone(1760, 'sine', now, 0.1);
      }

      // Clean up context after sound triggers
      setTimeout(() => { if (ctx.state !== 'closed') ctx.close(); }, 1000);
    } catch (e) { console.error(e); }
  }, [state.notificationSound, state.customSoundData]);

  // Reminder Check Loop
  useEffect(() => {
    const checkReminders = () => {
      const now = Date.now();
      // Filter tasks that have pending reminders in the past
      const tasksWithDueReminders = state.tasks.filter(t =>
        !t.isCompleted && !t.isArchived && t.reminders && t.reminders.some(r => r <= now)
      );

      if (tasksWithDueReminders.length > 0) {
        // We only alert for the first one to avoid spam, others will get caught in next cycle if we handle them
        const task = tasksWithDueReminders[0];
        ...
        // Show notification
        setActiveAlert({
          title: t('reminder_prefix'),
          body: `${task.title} - ${t('reminder_body_default')}`
        });

        // Remove the triggered reminders from state so they don't trigger again
        setState(prev => ({
          ...prev,
          tasks: prev.tasks.map(t => {
            if (tasksWithDueReminders.some(tr => tr.id === t.id)) {
              return {
                ...t,
                reminders: t.reminders.filter(r => r > now)
              };
            }
            return t;
          })
  ...
  }, [state.tasks, playNotificationSound, t]);
    ...
    try {
      ...
      if (Array.isArray(parsed.tasks)) {
        setState(prev => ({
          ...
          inboxFilter: parsed.inboxFilter || prev.inboxFilter
        }));
  ...
  const getFilteredTasks = () => {
    ...
    switch (state.viewMode) {
      case 'inbox':
        // If filter is flexible, show everything without a project (even if it has a deadline)
        // If strict, show only if no project AND no deadline
        if (state.inboxFilter === 'flexible') {
          return state.tasks.filter(t => !t.isArchived && !t.projectId);
        }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transition-all duration-300 ease-in-out lg:static lg:translate-x-0 ${state.isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <Sidebar
            ...
            <>
              ...
              {state.viewMode === 'settings' && (
                <Settings
                  ...
                  inboxFilter={state.inboxFilter || 'strict'}
                  onInboxFilterChange={updateInboxFilter}
                />
              )}
            </>
          )}
