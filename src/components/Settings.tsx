...
import { Download, Upload, Cloud, Monitor, Smartphone, Moon, Sun, Laptop, Languages, Volume2, Play, Music, Trash2, Inbox } from 'lucide-react';
...
interface SettingsProps {
  ...
  onPreviewSound: () => void;
  inboxFilter: 'strict' | 'flexible';
  onInboxFilterChange: (filter: 'strict' | 'flexible') => void;
}
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8">
        ...
        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-lg overflow-x-auto">
          {(['en', 'fr', 'de', 'es'] as const).map(lang => (
            <button
              ...
              className={`flex-1 min-w-[80px] flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-medium transition-all ${language === lang
      ...
      <section className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 mb-8 transition-colors">
        <div className="flex items-center gap-3 mb-4 text-indigo-500 dark:text-indigo-400">
          <Inbox size={24} />
          <h3 className="text-lg font-semibold">{t('inbox_config') || 'Inbox Configuration'}</h3>
        </div>

        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          {t('inbox_filter_desc') || 'Choose which tasks appear in the Inbox.'}
        </p>

        <div className="flex flex-col sm:flex-row gap-3">
          <button
            onClick={() => onInboxFilterChange('strict')}
            className={`flex-1 flex flex-col items-center justify-center p-4 rounded-lg border text-sm transition-all ${inboxFilter === 'strict'
          ...
          >
            <span className="font-semibold">{t('inbox_filter_strict_title')}</span>
            <span className="text-xs opacity-70 mt-1">{t('inbox_filter_strict_desc')}</span>
          </button>

          <button
            onClick={() => onInboxFilterChange('flexible')}
            className={`flex-1 flex flex-col items-center justify-center p-4 rounded-lg border text-sm transition-all ${inboxFilter === 'flexible'
              ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
              : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
              }`}
          >
            <span className="font-semibold">{t('inbox_filter_flexible_title')}</span>
            <span className="text-xs opacity-70 mt-1">{t('inbox_filter_flexible_desc')}</span>
          </button>
        </div>
      </section>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium transition-all ${notificationSound === 'default'
                ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
                : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium transition-all ${notificationSound === 'retro'
                ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
                : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium transition-all ${notificationSound === 'soft'
                ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
                : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium transition-all ${notificationSound === 'custom'
                ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300'
                : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                }`}
            >
        ...
        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-lg">
          {(['light', 'dark', 'system'] as const).map(theme => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-medium transition-all ${currentTheme === theme
                ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm'
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                }`}
            >
