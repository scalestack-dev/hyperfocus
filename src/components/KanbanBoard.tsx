          ...
          <div className="flex items-center gap-1">
            {!task.isCompleted && (
              <button
                ...
                className={`p-1 rounded-full transition-colors ${activeTaskId === task.id
        ...
        <div className="h-full flex px-4 pb-4 pt-2 gap-4 min-w-max">
          {columns.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`flex flex-col w-80 h-full rounded-xl transition-colors ${isDragOver ? 'bg-indigo-50 dark:bg-slate-700/80 ring-2 ring-indigo-400' : 'bg-slate-100/50 dark:bg-slate-800/30'
                  }`}
